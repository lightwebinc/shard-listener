...
//
// ROLLOUT ORDER — the same note is in retry-endpoint and shard-manifest:
//
...
// BeaconGroupPrefixes resolves the prefixes for this listener's beacon
// group joins (BRC-129 index 0xFFFD: BRC-126 ADVERTs plus BRC-139
// manifests) from -control-group-compat, -source-mode and -beacon-scope.
func (c *Config) BeaconGroupPrefixes() ([]uint16, error) {
	return ControlGroupPrefixes(c.ControlGroupCompat, c.SourceMode, c.BeaconScope)
}

// AnnounceGroupPrefixes resolves the prefixes for this listener's BRC-127
// subtree-group-announce joins (BRC-129 index 0xFFFC) at one -announce-scope
// entry. Same derivation, same switch: 0xFFFC is a control-plane group and
// BRC-129's trailing note covers it identically.
//
// Unlike the beacon, the sender side of this group was ALREADY conformant —
// shard-proxy emits announcements via its -source-mode-derived prefix — so
// on an SSM fabric the listener's FF0x-only join meant no announcement ever
// arrived. There is no flag day here, only a mismatch to close; "both" (the
// default) closes it without assuming which prefix a given proxy uses.
func (c *Config) AnnounceGroupPrefixes(scopeName string) ([]uint16, error) {
	return ControlGroupPrefixes(c.ControlGroupCompat, c.SourceMode, scopeName)
}
