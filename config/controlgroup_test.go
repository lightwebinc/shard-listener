...
// a live mismatch.
func TestAnnounceGroupPrefixes_SSMMatchesTheProxyEmit(t *testing.T) {
	c := &Config{SourceMode: "ssm", ControlGroupCompat: ControlGroupDerived}
	got, err := c.AnnounceGroupPrefixes("site")
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 1 || got[0] != 0xFF35 {
		t.Fatalf("announce prefixes = %#04x, want [0xff35]", got)
	}
	addr := shard.GroupAddr(got[0], shard.DefaultGroupID, shard.GroupSubtreeGroupAnnounce)
	if !addr.Equal(net.ParseIP("ff35::b:fffc")) {
		t.Errorf("announce group = %s, want ff35::b:fffc", addr)
	}
}

// The default joins both, so a listener works against a proxy on either
// prefix without the operator having to know which.
func TestAnnounceGroupPrefixes_DefaultJoinsBoth(t *testing.T) {
	c := &Config{SourceMode: "ssm", ControlGroupCompat: ControlGroupBoth}
	got, err := c.AnnounceGroupPrefixes("site")
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 2 || got[0] != 0xFF05 || got[1] != 0xFF35 {
		t.Fatalf("announce prefixes = %#04x, want [0xff05 0xff35]", got)
	}
}
